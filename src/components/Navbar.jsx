...
const [isopenbar, setIsopenbar] = useState(false)
  return (
    <div className={`w-screen h-16 flex fixed items-center p-5 md:p-0  justify-between md:justify-around transition-colors ease-linear duration-700 text-gray-400 z-50 ${islightmode ? "text-black bg-white":'text-white bg-black'}`}>
      ...
      <button
        className={`border md:hidden h-8 w-8 rounded-lg border-gray-700 ${isopenbar ? " bg-black/10":""}`} onClick={()=>setIsopenbar(!isopenbar)}
      >
        {!isopenbar?<i className="fa-solid fa-bars"></i>:<i class="fa-solid fa-xmark"></i>}
      </button>
      </div>
      <div className={`absolute top-16 left-0 w-full h- z-50 transition-transform duration-300 ${
  !isopenbar ? "hidden" : ""
}  backdrop-blur-lg backdrop-saturate-150 border-l  shadow-xl ${islightmode?" border-black/30 bg-black/50":"border-white/20 bg-white/10"}`}>
  
  <ul className=' flex-col text-sm font-semibold text-white gap-3 p-2 ml-2'>
        {['home', 'about', 'projects', 'contact'].map((section) => (
          <ScrollLink
            key={section}
            to={section}
